...
+ [Closure Library](https://github.com/google/closure-library) is a separate
    ...
    code, see [Closure Library References][].
...
+ `myphysicslab` – top-level directory (can be named whatever you like)
    ...
    + `build` – *(generated by `make`)* – Directory created by the command
      `make COMPILE_LEVEL=simple`. Contains compiled applications and HTML files.
        ...
        + `index_de.html` – links to all the example HTML applications, German versions
...
+ Every application has an HTML file to run the application. This is intended to
    be for testing, not as a finished web page.
...
for developer testing, not meant to be finished web pages. The command to build them
is

    make
    open build/index_en.html

The _HTML Example Files_ are listed in `src/index.html` for quick access and linked
together via **"next" and "previous" links** on each page to easily view the entire set
files. Clicking on the myPhysicsLab logo on any page takes you back to the index files.

After compiling, there will be a **version for each locale** such as `index_en.html`
and `index_de.html`. To build only the English versions of the apps use this command:
...
Note that **the links only work in the compiled versions**, not when
[Debugging (running uncompiled)][].

The "next" and "previous" links are auto-generated by the `prep_html.pl` perl script,
which gets the order by reading the file `src/index_order.txt`.
