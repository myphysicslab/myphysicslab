...
+ `myphysicslab` – top-level directory (can be named whatever you like)
    ...
    + `closure-library` – a symbolic link needed when running from uncompiled (source
        code) HTML files. **Create this manually**, see [Environment Setup][].
    ...
    + `MachineName.js` – *optional file* – if present it defines the global property
        `MYPHYSICSLAB_MACHINE_NAME` which is used to find expected times in
        performance tests. See [Performance Tests][].

    + `makefile` – the instructions used by `make`

    + `myConfig.mk` – Specifies information that is unique to each development
       environment, such as where tools are located, build options, etc.
       **Create this manually, by copying from `sampleConfig.mk`**
    ...
    + `README.md` – Dossier transforms this to be the top-level `index.html` file in
        `docs` directory, so it is the start page for documentation.
...
+ Java, for running Closure Compiler and Dossier

+ Perl, for running `prep_html.pl`
...
+ `closure-library/closure/bin/build/depswriter.py` – Python program which creates
    the `deps.js` needed to run from uncompiled source code and create documentation.

+ `DOSSIER` – (`make` variable specified in `myConfig.mk`) Java program that creates
    documentation from Javascript files.

The following instructions assume you want to build from the myPhysicsLab source code
using Closure Compiler.

1. Copy the file `sampleConfig.mk` to `myConfig.mk` and edit the latter to specify
    location of Closure Compiler (and Dossier if building documentation).

2. Create a symbolic link to `closure-library`.  This is used when
    [Debugging (running uncompiled)][] and for building
    documentation. Here is an example of how to create the symbolic link:

    ```
    $ ln -s ../closure-library/ closure-library
    ```

3. Execute `make` at the command line with your directory set to the `myphysicslab`
    directory. This will compile all applications and tests in all language versions
    (using default option `COMPILE_LEVEL=simple`). Execute `make help` to see available
    options.

4. (Optional) Copy the file `sampleMachineName.js` to `MachineName.js` and edit
    the file to have a unique name for your development machine, for running
    [Performance Tests][].

5. (Optional) Execute `make docs` for [Building Documentation][].
...
+ `LOCALE` – selects which locale to use for [Internationalization (i18n)][], but only
    ...
    To build all apps for a given locale use targets `apps-en` or `apps-de`.
...
[Dossier website](https://github.com/jleyba/js-dossier). Then specify the location of
`dossier.jar` on your system by editing the `DOSSIER` variable in your version of
`myConfig.mk`. Note that some options are specified in the file `dossier_config.json`.
After installing Dossier, you should be able to execute this command (the path to
`dossier.jar` is likely to be different on your system):
...
`docs/Overview.md` and `docs/Engine2D.md`.
...
The file `docs/index.html` is the **home file** for the documentation, it is built by
Dossier from the `README.md` markdown file.

To build only the markdown documentation (overview and engine2D) use the command
...
in the (optional) file `MachineName.js` at the root level. This `MachineName.js` file is
not checked in to the source repository because it is different on each machine. There
is a file `sampleMachineName.js` which can be copied and modified to specify the unique
machine name.
...
When you look at the [Start-Up HTML Files](Architecture.html#start-uphtmlfile) for applications, you will notice a mix of regular HTML and macros, for example:
...
The macros are the words starting with a hash-tag.

During the build process, the `.html` file undergoes text manipulation to expand macro
definitions (roughly similar to macros in the C preprocessor). The text manipulation is
done with a **perl script called `prep_html.pl`**. That perl script interprets
text-replacement rules defined in the `macros.html` and `macros_tab.html` files.
