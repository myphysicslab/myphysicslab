...
import { RollerDoubleSim } from './RollerDoubleSim.js';
...
/** Creates the {@link RollerDoubleSim} simulation

*/
export class RollerDoubleApp extends AbstractApp<RollerDoubleSim> implements Subject, SubjectList, Observer {
...
constructor(elem_ids: ElementIDs) {
  ...
  let pn = sim.getParameterNumber(RollerDoubleSim.en.GRAVITY);
...
@param simRect
*/
setSimRect(simRect: DoubleRect) {
  this.simRect = simRect;
  this.simView.setSimRect(simRect);
};

} // end class
Util.defineGlobal('sims$roller$RollerDoubleApp', RollerDoubleApp);
