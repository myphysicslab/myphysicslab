...
import { makeNGon } from './TestShapes.js';
import { Util } from '../lab/util/Util.js';
import { Vector } from '../lab/util/Vector.js';
import { Walls } from '../lab/engine2D/Walls.js';

import { assertEquals, schedule, startTest, assertThrows,
    assertTrue, assertFalse, assertRoughlyEquals, setTestName,
    perfExpected, perfResult, getPerfLimit, reportTestResults }
    from './TestRig.js';
import { makeVars, setBodyVars, runTest, checkContactDistances, checkValue }
    from './Engine2DTestRig.js';

const groupName = 'StraightStraightTest.';
...
*/
export class StraightStraightTest {
...
@param damping
*/
private static commonSetup1(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>, damping?: number) {
...
@param advance
*/
static six_blocks_settle_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
...
*/
static six_blocks_settle(): void {
  setTestName(groupName+'six_blocks_settle');
...
@param advance
*/
static six_blocks_settle_2_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
  StraightStraightTest.six_blocks_settle_setup(sim, advance);
...
at all contacts.
*/
static six_blocks_settle2(): void {
  setTestName(groupName+'six_blocks_settle2');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
...
specifically the `UtilCollision.subsetCollisions1()` method was invented
...
@param advance
*/
static six_blocks_1_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
  StraightStraightTest.commonSetup1(sim, advance);
  const zel = Walls.make(sim, /*width=*/12.0, /*height=*/12.0, /*thickness=*/1.0);
  let block1;
  for (let i=0; i<6; i++) {
...
*/
static six_blocks_1(): void {
  setTestName(groupName+'six_blocks_1');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
...
*/
static six_blocks_2_setup(sim: ContactSim, advance:CollisionAdvance<RigidBodyCollision>) {
  StraightStraightTest.commonSetup1(sim, advance);
  const zel = Walls.make(sim, /*width=*/12.0, /*height=*/12.0, /*thickness=*/1.0);
  let block1;
  for (let i=0; i<6; i++) {
...
*/
private static six_blocks_2(): void {
  setTestName(groupName+'six_blocks_2');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
...
*/
static six_blocks_3_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
...
/** Tests scenario with six blocks that causes a matrix solve exception.
*/
private static six_blocks_3(): void {
  setTestName(groupName+'six_blocks_3');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
...
* @param wallsFirst
*/
private static setup_six_blocks(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>, v: number[], wallsFirst?: boolean) {
...
@param advance
*/
static six_blocks_4_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
...
@package
*/
static six_blocks_4(): void {
  setTestName(groupName+'six_blocks_4');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
...
@param advance
*/
static six_blocks_5_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
  StraightStraightTest.setup_six_blocks(sim, advance,
  [
    -5.489965075066497,
...
*/
private static six_blocks_5(): void {
  setTestName(groupName+'six_blocks_5');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
...
@param advance
*/
static six_blocks_6_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
  StraightStraightTest.setup_six_blocks(sim, advance,
  [
    -2.479254054598802,
...
/**
*/
private static six_blocks_6(): void {
  setTestName(groupName+'six_blocks_6');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
...
@param advance
*/
static six_blocks_7_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
  StraightStraightTest.setup_six_blocks(sim, advance,
  [
    -1.4008804821505663,
...
/**
*/
private static six_blocks_7(): void {
  setTestName(groupName+'six_blocks_7');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
...
@param advance
*/
static six_blocks_8_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
  StraightStraightTest.setup_six_blocks(sim, advance,
  [
    -1.4006667035152596,
...
*/
private static six_blocks_8(): void {
  setTestName(groupName+'six_blocks_8');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
...
@param advance
*/
static six_blocks_9_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
  StraightStraightTest.setup_six_blocks(sim, advance,
  [
    -1.4098834956330757,
...
/**
*/
private static six_blocks_9(): void {
  setTestName(groupName+'six_blocks_9');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
...
@param advance
*/
static six_blocks_10_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
  StraightStraightTest.setup_six_blocks(sim, advance,
  [
    -4.112367787413438,
    0.39026511526212854,
...
/**
*/
private static six_blocks_10(): void {
  setTestName(groupName+'six_blocks_10');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
...
@param advance
*/
static six_blocks_11_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
  StraightStraightTest.setup_six_blocks(sim, advance,
  [
    -3.4807418124962246,
...
/**
*/
private static six_blocks_11(): void {
  setTestName(groupName+'six_blocks_11');
...
@param advance
*/
static six_blocks_12_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
  StraightStraightTest.setup_six_blocks(sim, advance,
  [
    -3.9795534864733946,
    0.5603947124399835,
...
/**
*/
private static six_blocks_12(): void {
  setTestName(groupName+'six_blocks_12');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
...
@param advance
*/
static hexagon_1_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
  StraightStraightTest.commonSetup1(sim, advance);
...
/**
*/
private static hexagon_1(): void {
  setTestName(groupName+'hexagon_1');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
...
@param advance
*/
static block_block_contact_1_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
  StraightStraightTest.commonSetup1(sim, advance);
...
are no collisions.
*/
private static block_block_contact_1(): void {
  setTestName(groupName+'block_block_contact_1');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
...
@param advance
*/
static block_block_contact_2_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
  StraightStraightTest.commonSetup1(sim, advance);
  const body0 = Shapes.makeBlock(1, 1, 'block1');
...
/**
*/
private static block_block_contact_2(): void {
  setTestName(groupName+'block_block_contact_2');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
...
*/
private static block_block_contact_2b(): void {
  setTestName(groupName+'block_block_contact_2b');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
  StraightStraightTest.block_block_contact_2_setup(sim, advance);
...
@param advance
*/
static ngon_block_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
  StraightStraightTest.commonSetup1(sim, advance);
  const body0 = makeNGon(16, 0.75);
...
/**
*/
static ngon_block(): void {
  setTestName(groupName+'ngon_block');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
...
@param advance
*/
static diamonds_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
  StraightStraightTest.commonSetup1(sim, advance);
...
*/
private static diamonds(): void {
  setTestName(groupName+'diamonds');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
  ...
  runTest(sim, advance, /*runUntil=*/7.0,
      /*expectedVars=*/vars, /*tolerance=*/0.00001);
  checkValue('energy', sim.getEnergyInfo().getTotalEnergy(), 0.8168246, 0.0001);
  // run for a few more seconds: there should be no more collision searches,
  // and energy should be constant
...
@param advance
*/
static one_block1_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
  StraightStraightTest.commonSetup1(sim, advance);
...
*/
static one_block1(): void {
  setTestName(groupName+'one_block1');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
...
@param advance
*/
static one_block2_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
  StraightStraightTest.commonSetup1(sim, advance);
...
/**
*/
static one_block2(): void {
  setTestName(groupName+'one_block2');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
...
@param advance
*/
static fast_close_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
  StraightStraightTest.commonSetup1(sim, advance);
  sim.setExtraAccel(ExtraAccel.NONE);
  const zel = Walls.make(sim, /*width=*/10, /*height=*/10);
  const block = Shapes.makeBlock(1, 3, 'blue');
...
/**
*/
static fast_close(): void {
  setTestName(groupName+'fast_close');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
...
@param advance
*/
static corner_collision_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
  StraightStraightTest.commonSetup1(sim, advance);
  advance.setTimeStep(0.01);
  const body0 = Shapes.makeDiamond(1, 1, Math.PI/4, 'diamond1');
...
/**
*/
static corner_collision(): void {
  setTestName(groupName+'corner_collision');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
...
@param advance
*/
static rounded_corner_collision_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
  StraightStraightTest.commonSetup1(sim, advance);
  advance.setTimeStep(0.01);
...
/**
*/
static rounded_corner_collision(): void {
  setTestName(groupName+'rounded_corner_collision');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
...
@param advance
*/
static oblique_corner_collision_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
  StraightStraightTest.commonSetup1(sim, advance);
...
/**
*/
static oblique_corner_collision(): void {
  setTestName(groupName+'oblique_corner_collision');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
...
@param advance
*/
static acute_corners_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
...
/**
*/
static acute_corners(): void {
  setTestName(groupName+'acute_corners');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
