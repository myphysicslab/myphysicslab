...
import { schedule, setTestName, reportTestResults, myPrintln, perfExpected, perfResult, getPerfLimit, testName} from './TestRig.js';
import { makeVars, setBodyVars, runTest, runExceptionTest, checkTightJoints }
    from './Engine2DTestRig.js';

const groupName = 'PileTest.';

/**  Tests involving piles of many objects with engine2D physics engine.
*/
export class PileTest {
...
@param advance
*/
static connected_blocks_pile_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
...
*/
static connected_blocks_pile_test(): void {
  setTestName(groupName+'connected_blocks_pile_test');
  ...
  checkTightJoints(sim, 0.025);
};

/** Stable resting configuration of blocks derived from a particular run of
{@link test/PileTest.PileTest.connected_blocks_pile_setup}.
@param sim
@param advance
*/
static stable_connected_blocks_pile_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
  sim.setSimRect(new DoubleRect(-3.5, -9.2, 3.5, -1));
  sim.addForceLaw(new DampingLaw(0.05, 0.15, sim.getSimList()));
  sim.setShowForces(false);
  sim.setCollisionHandling(CollisionHandling.SERIAL_GROUPED_LASTPASS);
...
{@link test/PileTest.PileTest.connected_blocks_pile_setup}.
So that we can compare running identical
configuration across different browsers and changes to myPhysicsLab software.
*/
static stable_connected_blocks_pile_test(): void {
  setTestName(groupName+'stable_connected_blocks_pile_test');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
...
{@link test/PileTest.PileTest.connected_blocks_pile_setup}
(just moments before the pile settles down).
@param sim
@param advance
*/
static near_stable_connected_blocks_pile_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
  sim.setSimRect(new DoubleRect(-3.5, -9.2, 3.5, -1));
  sim.addForceLaw(new DampingLaw(0.05, 0.15, sim.getSimList()));
  sim.setShowForces(false);
  sim.setCollisionHandling(CollisionHandling.SERIAL_GROUPED_LASTPASS);
...
*/
static near_stable_connected_blocks_pile_test(): void {
  setTestName(groupName + 'near_stable_connected_blocks_pile_test');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
...
@param advance
*/
static pile_config_1_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
...
to May 12, 2016.
*/
static pile_config_1_test(): void {
  setTestName(groupName+'pile_config_1_test');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
...
* @param advance
*/
static pile_10_random_blocks_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
  sim.setSimRect(new DoubleRect(-3.5, -9.2, 3.5, -1));
  sim.addForceLaw(new DampingLaw(0.05, 0.15, sim.getSimList()));
  sim.setShowForces(false);
  sim.setCollisionHandling(CollisionHandling.SERIAL_GROUPED_LASTPASS);
...
*/
static pile_10_random_blocks(): void {
  setTestName(groupName+'pile_10_random_blocks');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
...
* @param gravity
*/
private static pile_make_v_pit(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>, damping: number, gravity: number) {
...
* @param advance
*/
static pile_uniform_balls_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
...
* @param advance
*/
static pile_uniform_blocks_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
  PileTest.pile_make_v_pit(sim, advance, /*damping=*/0.05, /*gravity=*/3.0);
  const rect = new DoubleRect(-9, 5, 9, 20);
...
@param advance
*/
static pile_20_random_blocks_setup(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
  sim.setSimRect(new DoubleRect(-3.5, -9.2, 3.5, -1));
  sim.addForceLaw(new DampingLaw(0.05, 0.15, sim.getSimList()));
  sim.setShowForces(false);
  sim.setCollisionHandling(CollisionHandling.SERIAL_GROUPED_LASTPASS);
...
*/
static pile_20_random_blocks(): void {
  setTestName(groupName+'pile_20_random_blocks');
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
...
* @param advance
*/
private static additive_pile_setup_(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>) {
  sim.addForceLaw(new DampingLaw(0.05, 0.15, sim.getSimList()));
  sim.setCollisionHandling(CollisionHandling.SERIAL_GROUPED_LASTPASS);
...
@param square  true means make a square block, false means make a round ball
@param start_num_blocks the number of blocks to quickly add at the start, which
  helps to speed up the running time of this test
@return the number of blocks at the point where performance started to degrade
*/
private static additive_pile_test(square: boolean, start_num_blocks: number): number {
  const sim = new ContactSim();
  const advance = new CollisionAdvance(sim);
  ...
  while (true) {
    ...
    myPrintln('num_bodies='+num_blocks
...
@param runUntil the simulation time to run until
@param square  true means make a square block, false means make a round ball
@return the elapsed real time that it took to run the simulation to the given time
*/
private static add_block_and_run(sim: ContactSim, advance: CollisionAdvance<RigidBodyCollision>, runUntil: number, square: boolean): number {
  const startTime = Util.systemTime();
...
*/
static additive_pile_square_test(expectedBlocks: number) {
  setTestName(groupName+'additive_pile_square_test');
  const startTime = Util.systemTime();
  ...
  myPrintln('additive_pile_square_test '+s);
  reportTestResults(num_blocks >= expectedBlocks, 'performance', s);
};
...
@param expectedBlocks
*/
static additive_pile_circle_test(expectedBlocks: number) {
  setTestName(groupName+'additive_pile_circle_test');
  const startTime = Util.systemTime();
  ...
  const s = 'reached '+num_blocks
        +' blocks; expected='+expectedBlocks
        +' totalTime='+Util.NF5(totalTime);
  myPrintln('additive_pile_circle_test '+s);
  reportTestResults(num_blocks >= expectedBlocks, 'performance', s);
};
