...
{@link sims/engine2D/RigidBodyObserver.RigidBodyObserver | RigidBodyObserver}.
```js
...
See {@link sims/engine2D/RigidBodyObserver.RigidBodyObserver | RigidBodyObserver}
which sets up several prototype objects.

### Drawing Name of MassObject

The name of the MassObject is drawn when the {@link setNameFont} property
has a valid font specifier string. There are also {@link setNameColor} and
{@link setNameRotate} properties that affect how the name is drawn.
Example code:
```js
...
The {@link setImageAT} AffineTransform can then be specified to cause the
...
{@link lab/engine2D/CircularEdge.CircularEdge | CircularEdge} for more information.
...
export class DisplayShape implements DisplayObject {
  ...
  * The thickness of the border is set by {@link thickness_}.
  */
  private strokeStyle_: string|undefined;
  /** Thickness of border drawn, see {@link strokeStyle_}, in screen
  ...
  * AffineTransform {@link imageAT_} is applied first, and the image is
  * clipped if {@link imageClip_} is set. This disables the name being
  * drawn.
  */
  ...
  * is drawn and the {@link image_} is drawn.
  * The AffineTransform {@link imageAT_} is applied first,
  * and the image is clipped if {@link imageClip_} is set.
...
/** Image to draw, after the massObject is filled and border is drawn. The
* AffineTransform {@link imageAT_} is applied first, and the image is
* clipped if {@link imageClip_} is set. This disables the name being drawn.
*/
...
/** Thickness of border drawn, see {@link getStrokeStyle},
...
* AffineTransform {@link imageAT_} is applied first, and the image is
* clipped if
* {@link imageClip_} is set. This disables the name being drawn.
*/
