...
goog.module('myphysicslab.lab.model.BodyVariable');
...
/** Represents a Variable for a RigidBody in a {@link myphysicslab.lab.model.VarsList}.
@implements {Variable}
*/
class BodyVariable {
/**
...
setFromString(value) {
  ...
  if (isNaN(v)) {
    throw 'not a number: '+value+' (BodyVariable.setFromString)';
  }
  this.setValue(v);
};
...
setValue(value) {
  if (this.getValue() != value) {
    //if (isNaN(value)) { throw 'NaN (BodyVariable.setValue)'; }
    this.setter_(value);
    this.seq_++;
...
setValueSmooth(value) {
  //if (isNaN(value)) { throw 'NaN (BodyVariable.setValueSmooth)'; }
  this.setter_(value);
};

} // end class
exports = BodyVariable;
